...
allowed-tools: Bash(git:*), Bash(gh:*), Read, Grep, SendMessage
user-invocable: true
---
...
3ファイル構成のチェックリストを読み込んでレビューの基準とする。
...
対応する GitHub Issue の受け入れ条件（Acceptance Criteria）を取得し、変更内容が各条件を満たしているか検証する。

#### Issue の特定

以下の優先順で対応 Issue を特定する:

1. **ユーザー指定あり** — `/code-review #123` のように Issue 番号が指定された場合はそれを使用
2. **ブランチ名から推測** — `feat/issue-11-auth-backend` のようにブランチ名に Issue 番号が含まれる場合は抽出して使用
3. **上記で特定できない場合** — ユーザーに確認を取る（「対応する Issue 番号を教えてください。受け入れ条件のチェックをスキップする場合はその旨お知らせください」）
...
Issue 本文から「受け入れ条件」セクション（`## 受け入れ条件` または `## Acceptance Criteria`）を抽出する。

#### チェック実施

受け入れ条件の各項目（`- [ ]` チェックボックス）について、diff およびコード内容から充足状況を判定する。
...
| # | 条件 | 判定 | 根拠 |
|---|------|------|------|
| 1 | <条件の要約> | OK / NG / 未確認 | <diff のどの部分で確認したか> |
| 2 | ... | ... | ... |

- **OK**: diff またはコードから条件を満たしていることが確認できた
- **NG**: 条件を満たしていない、または実装が不足している
- **未確認**: 静的レビューでは判定不可（実行環境・DB 依存など）
```

**NG 項目がある場合**: Must 指摘としてレビュー結果に含める。
...
- [shared/REVIEW_CHECKLIST_COMMON.md](../shared/REVIEW_CHECKLIST_COMMON.md) - 共通チェックリスト（**常に参照**）
- [shared/REVIEW_CHECKLIST_BACKEND.md](../shared/REVIEW_CHECKLIST_BACKEND.md) - バックエンドチェックリスト
- [shared/REVIEW_CHECKLIST_FRONTEND.md](../shared/REVIEW_CHECKLIST_FRONTEND.md) - フロントエンドチェックリスト
